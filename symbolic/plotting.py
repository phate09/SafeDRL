...
def plot(environment_name, storage: prism.state_storage.StateStorage, *, plot_type="lb", folder_path, file_name_save: str = None):
    ...
    if plot_type == "lb":
        ...
        if state_size > 2:
            results = [(x, prob) for x, prob in results if x[3][0] <= 0 <= x[3][1] and x[2][0] <= 0 <= x[2][1]]
        utils.show_heatmap(results, title="Heatmap of the upper bound measured at the initial state", save_to=save_path, rounding=3)
...
if __name__ == '__main__':
    ...
    leaves = storage.get_leaves(shortest_path, 0.8)
    ...
    plot(environment_name, storage, folder_path=folder_path, plot_type="lb", file_name_save=f"{environment_name}_e{rounding}_h{horizon}_lb.svg")
