    ...
    def concretize(self):
        ...
        if self.edep:
            ...
            for i in range(len(self.edep)):
                e = e + self.edep_ind[i].t().mm(self.edep[i].abs())  # print("sym e2", e)
...
class Symbolic_interval_proj1(Interval):
    ...
    * :attr:`edep` keeps the error dependency introduced by each
    ...
    def __init__(self, lower, upper, proj=None, proj_ind=None, use_cuda=False):
        ...
        self.batch_size = self.c.shape[0]
        if (self.use_cuda):
            self.idep = torch.eye(self.n, device=self.c.get_device())
        else:
        ...
        if (proj_ind is None):
            ...
            self.idep_proj = self.idep_proj * self.e.view(self.batch_size, self.input_size)
            self.e = self.e.view(self.batch_size, self.input_size)[:, idep_ind]
        else:
            self.idep = self.idep.unsqueeze(0) * self.e.view(self.batch_size, 1, self.n)
            # print(self.idep.shape, proj_ind.shape)
            self.idep = self.idep.gather(index=proj_ind.unsqueeze(-1).repeat(1, 1, self.n), dim=1)
            # print(self.idep.shape)

            self.idep_proj = (self.idep.sum(dim=1) == 0).type_as(self.idep)
            self.idep_proj = self.idep_proj * self.e.view(self.batch_size, self.input_size)  # print("proj",self.idep_proj.shape)
    ...
    def concretize(self):
        ...
        if (self.proj_ind is None):
            e = (self.idep * self.e.view(self.batch_size, self.proj, 1)).abs().sum(dim=1)
        else:
    ...
    def worst_case(self, y, output_size):
        ...
        idep_t = self.idep.masked_select(kk.view(self.batch_size, 1, output_size)).view(self.batch_size, self.proj, 1)
        self.idep = self.idep - idep_t
