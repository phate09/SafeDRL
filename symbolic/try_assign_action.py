...
def try2():
    ...
    subwindows = [window]#DomainExplorer.box_split_tuple(window, rounding)
    while len(subwindows) != 0:
        ...
        if len(filtered) != 0:
            ...
            if all_same and not math.isclose(area_filtered, area_sub, abs_tol=1e-10):  # check for empty areas
                all_same = False
            if all_same:
