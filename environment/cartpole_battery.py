...
class CartPoleBatteryEnv(gym.Env):
    """
    Description:
    ...
    def reset(self):
        ...
        self.state[4] = 100.0
        self.state[0:4] = self.np_random.uniform(low=-0.05, high=0.05, size=(4,))
        if self.phase == 1:
            self.state[3] = self.np_random.uniform(-0.5, 0.5)  # thetadot
            self.state[2] = self.np_random.uniform(-0.2, 0.2)  # theta
